...
Figure~\ref{fig:ic_corr_D1} shows that as the size of time correlation \(\rho\) approaches 1 the information content in the two observations of \(C_{fol}\) grows for both SIC and \(dfs\). This increase in information content makes sense as including the correlation in time is increasing the influence of each observation of the analysis update. 
...
In this section we repeat and extend some of the results we have found for DALEC1 state estimation information content to the joint parameter and state estimation DALEC2 system. This means we now have an augmented state of 23 elements (17 parameters and 6 state variables) as opposed to the 5 state members for DALEC1. This means we can no longer examine the analytic representations of information content and instead look at the information content calculated analytically for DALEC2. 



In section~\ref{sec:info_con_single_time}
