...
\begin{figure}[ht]
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        ...
        \caption{Temperature term for years data}
        \label{fig:temp_nee_oregon2007}
    \end{subfigure}
    \caption{SIC for a single NEE observation changing throughout a years window using driving data from a pine stand in Oregon taken in 2007 (left). Temperature term \(e^{\Theta T^{i}}\) (where \(T^{i}\) is the mean daily temperature) using the same driving data (right).}
...
\hat{\mathbf{R}}=
\begin{pmatrix}
\mathbf{R}_0 & \sigma_{1,2}^{2}  \\
\sigma_{2,1}^{2} & \mathbf{R}_1  \\
\end{pmatrix}
=
\begin{pmatrix}
\sigma_{cfol,o}^{2} & \sigma_{1,2}^{2}  \\
\sigma_{2,1}^{2} & \sigma_{cfol,o}^{2}  \\
\end{pmatrix},
\end{equation}
here \(\hat{\mathbf{R}}\) is symmetric and we have \(\sigma_{1,2}^{2} = \sigma_{2,1}^{2} = \sigma_{corr}^{2}\). We then find,
\begin{equation}
SIC = \frac{1}{2}\text{ln}\frac{\begin{vmatrix} \mathbf{B} \end{vmatrix}}{\begin{vmatrix} \mathbf{A} \end{vmatrix}} =\frac{1}{2}\text{ln} \bigg(1+\frac{\sigma_{cfol,b}^{2}}{\sigma_{cfol,o}^{2} - \frac{\sigma_{corr}^{2}}{\sigma_{cfol,o}^{2}}}+\frac{\sigma_{cfol,b}^{2}\eta_0^{2}}{\sigma_{cfol,o}^{2} - \frac{\sigma_{corr}^{2}}{\sigma_{cfol,o}^{2}}} - \frac{2\sigma_{cfol,b}^{2}\sigma_{corr}\eta_0}{\sigma_{cfol,o}^{4} - \sigma_{corr}^{2}} \bigg), \label{eq:sic_2cfolcorr}
\end{equation}
We have a similar expression as in equation~\eqref{eq:sic_2cfol} qith no correlations. Terms 2 and 3 in equation~\eqref{eq:sic_2cfolcorr} will be slightly increased as \(\frac{\sigma_{corr}^{2}}{\sigma_{cfol,o}^{2}}\) has been subtracted from the denominator of each term. These terms will only be slightly increased as \(\frac{\sigma_{corr}^{2}}{\sigma_{cfol,o}^{2}}\) is bound above by 1 and will tend to be smaller as the correlation between the two successive observation errors will tend to be closer to zero than 1. As we have an additional \(4^{\text{th}}\) term in equation~\eqref{eq:sic_2cfolcorr} subtracted from the other 3 terms the included time error correlation has the overall effect of reducing the information content in the two successive \(C_{fol}\) observations. The reduction in information content in successive observations makes sense as we are assimilating less independent information. This backs up the results seen in the previous chapter REF where including correlations in time between observations of NEE reduces overfitting to the observations in the assimilation window and gives an improvement to the model forecast.
