...
\usepackage{bm,amsmath,bbm,amsfonts,nicefrac,latexsym,amsmath,amsfonts,amsbsy,amscd,amsxtra,amsgen,amsopn,bbm,amsthm,amssymb,graphicx, pbox}
\usepackage{fancyhdr}
...
\end{equation}
where all terms have the same meaning as describing in section~\ref{sec:dalec2}. For total daytime NEE we have,
\begin{equation}
...
\end{equation}
where \(\delta_{day}\) is the day length, expressed as \(\frac{\text{number of daylight hours}}{24}\), and \(T_{day}^{i}\) is the mean daytime temperature. For nighttime NEE we have,
\begin{equation}
...
Mensuration measurements for West of Forest from Ian, 
