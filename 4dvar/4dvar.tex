...
\end{equation}
where $\textbf{h}$ is a vector of unit length, which we can take to be $\nabla J(\textbf{x}_0)||\nabla J(\textbf{x}_0)||^{-1}$. Maybe also consider $\textbf{h}=\textbf{x}_0||\textbf{x}_0||^{-1}$?? In plot REF we can see that as $\alpha \rightarrow 0$ we have $f(\alpha) - 1 \rightarrow 1$ before $\alpha$ gets too close to machine zero. This gives us confidence that the gradient of our cost function is behaving as expected.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.8\textwidth]{testgradcost.png}
    \caption{Test of the gradient of the cost function. As $\alpha \rightarrow 0$ we have $abs(1 - f(\alpha)) \rightarrow 0$ up to $O(\alpha) = 10^{-4}$ where we have gone past the precision of the computer.}
